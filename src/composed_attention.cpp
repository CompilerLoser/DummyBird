    ...
    for (int i = 0; i < length; ++i)
        for (int j = 0; j < random_size; ++j)
        {
            ...
            while (
                ...
                (rand_cols[j] < window_index[i] + window_size) && (rand_cols[j] > window_index[i]) ||
                       ...
                       int random_size, float *res)
{
    ...
    for (int i = 0; i < QL - local_height; ++i)
        window_index[i] = start_padding + int(i / window_height) * window_stride;
    ...
    for (int i = 0; i < QL - local_height; ++i)
    {
        ...
        for (int j = 0; j < local_width; ++j)
            for (int l = 0; l < HL; ++l)
                scores[i * score_len + j] += Q[idx * HL + l] * K[j * HL + l];
        for (int j = local_width; j < local_width + window_size; ++j)
            for (int l = 0; l < HL; ++l)
                scores[i * score_len + j] += Q[idx * HL + l] * K[(window_index[i] + j - local_width) * HL + l];
        for (int j = local_width + window_size; j < score_len; ++j)
            for (int l = 0; l < HL; ++l)
                scores[i * score_len + j] += Q[idx * HL + l] * K[(rand_cols[i * random_size + j - local_width - window_size]) * HL + l];
    }
